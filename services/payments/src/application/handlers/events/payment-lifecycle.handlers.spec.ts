...
} from './payment-lifecycle.handlers';
...
describe('the lifecycle handlers', () => {
  ...
  async function handlers() {
    ...
    return {
      approved: moduleRef.get(PaymentApprovedHandler),
      declined: moduleRef.get(PaymentDeclinedHandler),
      compensated: moduleRef.get(PaymentCompensatedHandler),
    };
  }
  ...
  it('queues an approval, rather than publishing it to the broker itself', async () => {
    ...
    await approved.handle(new PaymentApprovedEvent(PAYMENT, MONEY, DEBIT, CREDIT, OCCURRED_AT));
  ...
  it('queues a compensation with the refund movement', async () => {
    ...
    await compensated.handle(new PaymentCompensatedEvent(PAYMENT, MONEY, REFUND, OCCURRED_AT));
  ...
  it('lets a failed write surface, so the surrounding transaction rolls back', async () => {
    ...
    await expect(
      approved.handle(new PaymentApprovedEvent(PAYMENT, MONEY, DEBIT, CREDIT, OCCURRED_AT)),
